...
- TypeScript 5.x
- パッケージマネージャー: npm
- フロントエンド: Web/PWA（スマホ最適化）
- 提供形態: Webサイト + PWA対応（ホーム画面追加）

### 段階実装方針

本プロジェクトは以下のフェーズで段階実装します：

- **Phase 0**: コンテンツ投入準備（動画10本程度、テスト用）
- **Phase 1 (MVP)**: 連続学習の核（ホーム + プレイヤー + 設定）
- **Phase 2**: カスタマイズ学習（動画本数・出題順の設定）
- **Phase 3**: 苦手だけ反復
- **Phase 4**: テスト範囲指定 + 周回
